...
namespace PodioPCL.Services
{
    public class AppMarketService
    {
        ...
        public AppMarketShares GetOwnShares(string type,int limit=6,int offset=0)
       {
        ...
        /// <returns></returns>
        public AppMarketShares GetSharesByAuthor(string type, int userId, int limit = 6, int offset = 0, string sort = "install")
        {
        ...
        /// <param name="sort">The sorting of the shares, either "install", "rating", "popularity", "recommended", "shared_on", or "name".Default value: install</param>
        /// <returns></returns>
        public AppMarketShares GetSharesByCategory(string type,int categoryId,int limit=6,int offset=0,string sort="install")
        {
        ...
        public int ShareApp(string refType, int refId, string name, string abstracts, string description, string language, int[] categoryId, int[] fileId, int videoId, List<string> features, int childrenId,string scope="public")
        {
        ...
        /// <param name="videoId">The youtube id of a introduction video, if any</param>
        public void UpdateShare(int shareId,string name,string abstracts,string description,string language,int[] categoryId,int[] fileId,int? videoId = null)
        {
            string url = string.Format("/app_store/{0}", shareId);
            dynamic requestData = new
            {
