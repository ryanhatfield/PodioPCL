...
namespace PodioPCL.Services
{
	...
	public class AppMarketService
	{
		...
		public Task<List<Organization>> GetOrgsWithPrivateShares()
		{
			string url = "/app_store/org/";
			return _podio.GetAsync<List<Organization>>(url);
		}
		...
		/// <returns>Task&lt;AppMarketShares&gt;.</returns>
		public Task<AppMarketShares> GetOwnShares(string type, int limit = 6, int offset = 0)
		{
			...
			return _podio.GetAsync<AppMarketShares>(url, requestData);
		}
		...
		/// <returns></returns>
		public Task<AppMarketShares> GetRecommendedShares(string type, string area)
		{
			string url = string.Format("/app_store/{0}/recommended/{1}/", type, area);
			return _podio.GetAsync<AppMarketShares>(url);
		}
		...
		/// <returns></returns>
		public Task<AppMarketShare> GetShare(int shareId)
		{
			string url = string.Format("/app_store/{0}/v2", shareId);
			return _podio.GetAsync<AppMarketShare>(url);
		}
		...
		/// <returns></returns>
		public Task<List<AppMarketShare>> GetShareByReference(string refType, int refId)
		{
			string url = string.Format("/app_store/{0}/{1}/", refType, refId);
			return _podio.GetAsync<List<AppMarketShare>>(url);
		}
		...
		/// <param name="limit">The maximum number of results to return Default value: 6</param>
		/// <param name="offset">The offset into the returned results Default value :0</param>
		/// <param name="sort">The sorting of the shares, either "install", "rating", "popularity", "recommended", "shared_on", or "name".Default value: install</param>
		/// <returns></returns>
		public Task<AppMarketShares> GetSharesByAuthor(string type, int userId, int limit = 6, int offset = 0, string sort = "install")
		{
		...
		/// <param name="sort">The sorting of the shares, either "install", "rating", "popularity", "recommended", "shared_on", or "name".Default value: install</param>
		/// <returns></returns>
		public Task<AppMarketShares> GetSharesByCategory(string type, int categoryId, int limit = 6, int offset = 0, string sort = "install")
		{
		...
		/// <param name="offset">The offset into the returned results Default value :0</param>
		/// <returns></returns>
		public Task<AppMarketShares> GetTopShares(string type, int limit = 6, int offset = 0)
		{
		...
		public async Task<int> ShareApp(string refType, int refId, string name, string abstracts, string description, string language, int[] categoryId, int[] fileId, int videoId, List<string> features, int childrenId, string scope = "public")
		{
		...
		public Task UnshareApp(int shareId)
		{
